...
public class OptimizedElementArray {
  ...
  private void setDefaultNullability() {
    ...
    final int end = UnsafeHolder.LONG_ARRAY_OFFSET + size();
    for (int offset = UnsafeHolder.LONG_ARRAY_OFFSET; offset < end; offset++) {
  ...
  private void setByte(long[] array, int offset, byte b) {
    if (UnsafeHolder.LONG_ARRAY_OFFSET != 0) {
      UnsafeHolder.getUnsafe().putByte(array, offset, b);
    } else {
  ...
  private byte getByte(long[] array, int offset) {
    if (UnsafeHolder.LONG_ARRAY_OFFSET != 0) {
      return UnsafeHolder.getUnsafe().getByte(array, offset);
    } else {
  ...
  public final void setType(int index, int snappyType) {
    setByte(this.primitives, UnsafeHolder.LONG_ARRAY_OFFSET + index,
        (byte)snappyType);
  }
  ...
  public final int getType(int index) {
    return getByte(primitives, UnsafeHolder.LONG_ARRAY_OFFSET + index);
  }
